...
if (!empty($series_desc)) {
    ...
    $fallback_tpl = srs_str('Stream <strong>%s</strong> with IPTV Anbieter. All seasons, all episodes. <strong>No restrictions.</strong>');
    $hero_subtitle_html = wp_kses_post(sprintf($fallback_tpl, esc_html($series_name)));
}

// ── Template strings ─────────────────────────────────────────────────────────
$hero_prefix = srs_str('Watch');
$hero_suffix = srs_str('Plus Thousands of Series with IPTV Anbieter');
