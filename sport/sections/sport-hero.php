...
if (!empty($sport_short_desc)) {
    ...
    $fallback_tpl = spl_str('Stream <strong>%s</strong> live with IPTV Anbieter. No cable required. <strong>No blackouts. No restrictions.</strong>');
    $hero_subtitle_html = wp_kses_post(sprintf($fallback_tpl, esc_html($sport_name)));
}

// ── Template strings ─────────────────────────────────────────────────────────
$hero_prefix = spl_str('Watch');
$hero_suffix = spl_str('& Thousands of Live Sport Events with IPTV Anbieter');
