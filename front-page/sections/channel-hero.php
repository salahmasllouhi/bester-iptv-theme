...
if (!empty($channel_short_desc)) {
    ...
    $fallback_tpl = tpl_str('Stream <strong>%s</strong> live with IPTV Anbieter. No cable required. <strong>No blackouts. No restrictions.</strong>');
    $hero_subtitle_html = wp_kses_post(sprintf($fallback_tpl, esc_html($channel_name)));
}

// ── Template strings ─────────────────────────────────────────────────────────
$hero_prefix = tpl_str('Watch');
$hero_suffix = tpl_str('Plus Thousands of Channels with IPTV Anbieter');
